...
public class Boss2Obj extends GameObj{
    ...
    public Boss2Obj(Image img, int width, int height, double x, double y, GameWin frame) {
        super(img, width, height, x, y, frame);
    }

    public Boss2Obj(Image img, double x, double y) {
        super(img, x, y);
    }

    public Boss2Obj(double x, double y) {
        super(x, y);
    }
    ...
    public void paintSelf(Graphics g) {
        ...
        if(GameWin.gameState==2) {
            if(GameWin.bossIfLive==1 && GameWin.bossCome==2){
                hp = 1000;
                x = 253.5-187.5;
                y = -304;
                GameWin.bossCome=0;
            }
            if(GameWin.bossIfLive==2){
                //血条
/*               g.setColor(Color.white);
                g.fillRect(gameWidth/2-100,50,200,10);*/
                g.setColor(Color.red);
                g.fillRect(gameWidth/2-100,50,hp*200/1000,10);
            }
            if(y<=0) {
                y += speed;
            }
