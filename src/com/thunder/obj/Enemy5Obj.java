...
public class Enemy5Obj extends GameObj{
    int hp=50;
    double speed=1;
    @Override
    public Image getImg() {
    ...
    public Enemy5Obj(Image img, int width, int height, double x, double y, GameWin frame) {
        super(img, width, height, x, y, frame);
    }

    public Enemy5Obj(Image img, double x, double y) {
        super(img, x, y);
    }

    public Enemy5Obj(double x, double y) {
        super(x, y);
    }
    ...
    public void paintSelf(Graphics g) {
        ...
        if(GameWin.gameState==2) {
            if (x < width * 0.2) {
                x += speed;
            } else if (x > gameWidth - 1.2 * width) {
                x -= speed;
            }
            if (this.hp<=0 || GameWin.boom==1) {
                MusicTool.playExplodeMusic();
                ExplosionMiddleObj explosionMiddleObj = new ExplosionMiddleObj(x, y);
                GameUtils.explosionMiddleObjList.add(explosionMiddleObj);
                GameUtils.removeObjList.add(explosionMiddleObj);
